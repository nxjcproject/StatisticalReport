    ...
    public class EnergyConsumptionMonthlyStatisticAnalysis
    {
        ...
        public static DataTable TableQuery(string _organizeID, string _date)//_temp表需要从张迪那获得
        {
            ...
            foreach (DataRow dr in _temp.Rows)
            {
                ...
                //处理电耗等数据
                //电耗-生料制备
                dr["ElectricityConsumption_RawBatch"] = MyToDecimal(dr["Output_RawBatch"]) != 0 ? Decimal.Parse((MyToDecimal(dr["Electricity_RawBatch"]) / MyToDecimal(dr["Output_RawBatch"])).ToString("#0.0000")) : 0;
                //电耗-熟料烧成
                dr["ElectricityConsumption_Clinker"] = MyToDecimal(dr["Output_Clinker"]) != 0 ? Decimal.Parse((MyToDecimal(dr["Electricity_Clinker"]) / MyToDecimal(dr["Output_Clinker"])).ToString("#0.0000")) : 0;
                //电耗-水泥制备
                dr["ElectricityConsumption_Cement"] = MyToDecimal(dr["Output_Cement"]) != 0 ? Decimal.Parse((MyToDecimal(dr["Electricity_Cement"]) / MyToDecimal(dr["Output_Cement"])).ToString("#0.00000")) : 0;
                //吨熟料综合电耗
                dr["ComprehensiveElectricityConsumption"] = MyToDecimal(dr["Output_Clinker"]) != 0 ?Decimal.Parse((MyToDecimal(dr["Electricity_RawBatch"]) + MyToDecimal(dr["Electricity_Clinker"])
                        / MyToDecimal(dr["Output_Clinker"])).ToString("#0.0000")) : 0;
                //吨熟料综合实物煤耗
                dr["ComprehensiveCoalConsumption"] = MyToDecimal(dr["Output_Clinker"]) != 0 ? Decimal.Parse((MyToDecimal(dr["Consumption_CoalDust"]) * 1000 / MyToDecimal(dr["Output_Clinker"])).ToString("#0.0000")) : 0;
                //吨熟料发电量
                dr["ComprehensiveElectricityOutput"] = MyToDecimal(dr["Output_Clinker"]) != 0 ? Decimal.Parse((MyToDecimal(dr["Output_Cogeneration"]) / MyToDecimal(dr["Output_Clinker"])).ToString("#0.0000")) : 0;
