...
namespace StatisticalReport.Service.ComprehensiveReport.DispatchDailyReport
{
    public static class DispatchDailyReportService
    {
        ...
        public static DataTable GetPlanAndTargetCompletionByLevelCode(DateTime date, string LevelCode, bool isShift)
        {
            ...
            string m_Sql_ActualMonth = @"Select B.VariableId, B.ValueType, sum(B.TotalPeakValleyFlatB) as Value
            ...
            string m_Sql_ActualDay = @"Select B.VariableId, B.ValueType, sum(B.TotalPeakValleyFlatB) as Value
                                from tz_Balance A , balance_Energy B, system_Organization C
                                where A.TimeStamp = '{0}'
                                and A.StaticsCycle = 'day'
                                and A.BalanceId = B.KeyID
                                ...
                                and (C.Type = '熟料' or C.Type = '水泥磨')
                                group by B.VariableId, B.ValueType";
            string m_Sql_ClinkerActualMonth = @"Select B.VariableId, B.ValueType, sum(B.TotalPeakValleyFlatB) as Value
                                from tz_Balance A , balance_Energy B, system_Organization C, system_Organization D
                                where A.TimeStamp like '{0}-%'
                                and A.StaticsCycle = 'day'
                                and A.BalanceId = B.KeyID
                                and B.OrganizationID in (D.OrganizationID)
                                and C.LevelCode = '{1}'
                                and C.Type = '水泥磨'
								and D.LevelCode like SUBSTRING(C.LevelCode,1,LEN(C.LevelCode) -2) + '%'
								and D.Type = '熟料'
                                group by B.VariableId, B.ValueType";
            string m_Sql_ClinkerActualDay = @"Select B.VariableId, B.ValueType, sum(B.TotalPeakValleyFlatB) as Value
                                from tz_Balance A , balance_Energy B, system_Organization C, system_Organization D
                                where A.TimeStamp like '{0}'
                                and A.StaticsCycle = 'day'
                                and A.BalanceId = B.KeyID
								...
								and D.Type = '熟料'
                                group by B.VariableId, B.ValueType";
            string m_Sql_OrganizationIds = @"select A.OrganizationID as OrganizationId from system_Organization A
                                where A.LevelCode like '{0}%'
                                and (A.Type = '熟料' or A.Type = '水泥磨')";
            ...
            m_Sql_ActualMonth = string.Format(m_Sql_ActualMonth, date.ToString("yyyy-MM"), LevelCode);
            m_Sql_ClinkerActualMonth = string.Format(m_Sql_ClinkerActualMonth, date.ToString("yyyy-MM"), LevelCode);   //当计算水泥磨时，需要计算熟料的平均电耗
            m_Sql_ActualDay = string.Format(m_Sql_ActualDay, date.ToString("yyyy-MM-dd"), LevelCode);
            m_Sql_ClinkerActualDay = string.Format(m_Sql_ClinkerActualDay, date.ToString("yyyy-MM-dd"), LevelCode);   //当计算水泥磨时，需要计算熟料的平均电耗
            m_Sql_OrganizationIds = string.Format(m_Sql_OrganizationIds, LevelCode);
            DataTable PlanResultTable = null;
            DataTable ActualDayResultTable = null;
            DataTable ActualMonthResultTable = null;
            DataTable ClinkerActualMonthResultTable = null;
            DataTable ClinkerActualDayResultTable = null;
            DataTable ConsumptionTempalte = null;
            DataTable OrganizationIdsTable = null;
            List<string> m_OrganizationIds = new List<string>();

            try
            {
                PlanResultTable = _dataFactory.Query(m_Sql_PlanResult); 
                ActualMonthResultTable = _dataFactory.Query(m_Sql_ActualMonth);
                ClinkerActualMonthResultTable = _dataFactory.Query(m_Sql_ClinkerActualMonth);
                ActualDayResultTable = _dataFactory.Query(m_Sql_ActualDay);
                ClinkerActualDayResultTable = _dataFactory.Query(m_Sql_ClinkerActualDay);
                ConsumptionTempalte = _dataFactory.Query(m_Sql_ConsumptionTemplate);
                OrganizationIdsTable = _dataFactory.Query(m_Sql_OrganizationIds);
            }
            catch
            {
            ...
            if (PlanResultTable != null)
            {
                ...
                foreach(DataRow RowItem in OrganizationIdsTable.Rows)
                {
                    m_OrganizationIds.Add(RowItem["OrganizationId"].ToString());
                }

                DataRow[] m_ComprehensiveResultRows = PlanResultTable.Select("CaculateType = 'Comprehensive' and (ValueType = 'ElectricityConsumption' or ValueType = 'CoalConsumption')");     //计算综合电耗的变量
                if (ActualMonthResultTable != null && ConsumptionTempalte != null)                      //填入月统计
                {
                    DataTable m_ConsumptionTempalteTempTable = ConsumptionTempalte.Copy();
                    ////计算电耗
                    DataTable m_NormalConsmptionResult = EnergyConsumption.EnergyConsumptionCalculate.Calculate(ActualMonthResultTable, m_ConsumptionTempalteTempTable, "ValueFormula", new string[] { "Value" });
                    SetCaculateValue(PlanResultTable, m_NormalConsmptionResult, "ActualMonth", "Value", "Normal");
                    /////计算产量
                    DataRow[] m_QuantityResultRows = ActualMonthResultTable.Select("ValueType = 'ElectricityQuantity' or ValueType = 'MaterialWeight'");     //计算综合电耗的变量
                    SetCaculateValue(PlanResultTable, m_QuantityResultRows.CopyToDataTable(), "ActualMonth", "Value", "Normal");
                    ////计算综合电耗、煤耗
                    Standard_GB16780_2012.Parameters_ComprehensiveData m_Parameters_ComprehensiveData = AutoSetParameters.AutoSetParameters_V1.SetComprehensiveParametersFromSql("day",
                        date.ToString("yyyy-MM-01"), DateTime.Parse(date.ToString("yyyy-MM-01")).AddDays(-1).ToString("yyyy-MM-dd"), m_OrganizationIds, _dataFactory);
                    ...
                    m_Function_EnergyConsumption_V1.LoadComprehensiveData(ClinkerActualMonthResultTable, m_Parameters_ComprehensiveData, "VariableId", "Value");
                    DataTable m_ComprehensiveDataTable = ActualMonthResultTable.Clone();
                    ...
                    m_Function_EnergyConsumption_V1.LoadComprehensiveData(ActualMonthResultTable, m_Parameters_ComprehensiveData, "VariableId", "Value");
                ...
                if (ActualDayResultTable != null && ConsumptionTempalte != null)                        //填入日统计
                {
                    DataTable m_ConsumptionTempalteTempTable = ConsumptionTempalte.Copy();
                    ////计算电耗
                    DataTable m_NormalConsmptionResult = EnergyConsumption.EnergyConsumptionCalculate.Calculate(ActualDayResultTable, m_ConsumptionTempalteTempTable, "ValueFormula", new string[] { "Value" });
                    SetCaculateValue(PlanResultTable, m_NormalConsmptionResult, "ActualDay", "Value", "Normal");
                    /////计算产量
                    DataRow[] m_QuantityResultRows = ActualDayResultTable.Select("ValueType = 'ElectricityQuantity' or ValueType = 'MaterialWeight'");     //计算综合电耗的变量
                    SetCaculateValue(PlanResultTable, m_QuantityResultRows.CopyToDataTable(), "ActualDay", "Value", "Normal");

                    ////计算综合电耗、煤耗
                    Standard_GB16780_2012.Parameters_ComprehensiveData m_Parameters_ComprehensiveData = AutoSetParameters.AutoSetParameters_V1.SetComprehensiveParametersFromSql("day",
                        date.ToString("yyyy-MM-dd"), date.ToString("yyyy-MM-dd"), m_OrganizationIds, _dataFactory);
                    Standard_GB16780_2012.Function_EnergyConsumption_V1 m_Function_EnergyConsumption_V1 = new Standard_GB16780_2012.Function_EnergyConsumption_V1();
                    m_Function_EnergyConsumption_V1.LoadComprehensiveData(ActualDayResultTable, m_Parameters_ComprehensiveData, "VariableId", "Value");
                    DataTable m_ComprehensiveDataTable = ActualDayResultTable.Clone();
