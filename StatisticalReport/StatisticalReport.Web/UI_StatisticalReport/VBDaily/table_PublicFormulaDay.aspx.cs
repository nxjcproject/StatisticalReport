...
{
    public partial class table_PublicFormulaDay : WebStyleBaseForEnergy.webStyleBase
    {
        private const string REPORT_TEMPLATE_PATH = "\\VBReportHeaderTemplate\\table_PublicFormulaDay.xml";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.OrganisationTree_ProductionLine.PageName = "table_PublicFormulaDay.aspx";                                     //向web用户控件传递当前调用的页面名称
        ...
        public static string GetReportData(string organizationId, string datetime,string reportType)
        {
            ...
            switch (reportType)
            {
                case "日报":
                    time = datetime;
                    myDataTable = PublicFormulaDay.TableQuery(organizationId, time);
                    break;
                case "月报":
                    time = datetime.Substring(0, 7);
                    myDataTable = PublicFormulaMonth.TableQuery(organizationId, time);
                    break;
                case "年报":
                    time = datetime.Substring(0, 4);
                    myDataTable = PublicFormulaYear.TableQuery(organizationId, time);
                    break;
            }
            //myDataTable = PublicFormulaDay.TableQuery(organizationId, datetime);
