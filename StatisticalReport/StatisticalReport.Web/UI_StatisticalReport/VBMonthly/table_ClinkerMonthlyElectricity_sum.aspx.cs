...
namespace StatisticalReport.Web.UI_StatisticalReport.VBMonthly
{
    public partial class table_ClinkerMonthlyElectricity_sum : WebStyleBaseForEnergy.webStyleBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.OrganisationTree_ProductionLine.PageName = "table_ClinkerMonthlyElectricity_sum.aspx";                                     //向web用户控件传递当前调用的页面名称
        ...
        public static string GetReportData(string organizationId, string datetime,string reportType)
        {
            ...
            switch (reportType)
            {
                case "月报":
                    ...
                    myDataTable = ClinkerMonthlyElectricity_sum.TableQuery(organizationId, time);
                    break;
                case "年报":
                    time = datetime.Substring(0, 4);
                    myDataTable = ClinkerYearlyElectricity_sum.TableQuery(organizationId, time);
                    break;
            }
            //myDataTable = ClinkerMonthlyElectricity_sum.TableQuery(organizationId, datetime);
