...
{
    public partial class table_ClinkerMonthlyOutput : WebStyleBaseForEnergy.webStyleBase
    {
        private const string REPORT_TEMPLATE_PATH = "\\VBReportHeaderTemplate\\table_ClinkerMonthlyOutput.xml";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.OrganisationTree_ProductionLine.PageName = "table_ClinkerMonthlyOutput.aspx";                                     //向web用户控件传递当前调用的页面名称
            ...
            if (m_FunctionName == "ExcelStream")
            {
                ...
                StatisticalReportHelper.ExportExcelFile("xls", "熟料生产线产量及消耗统计月报表.xls", m_HtmlData);
            }
        }
        ...
        public static string GetReportData(string organizationId, string datetime,string reportType)
        {
            ...
            switch (reportType)
            {
                case "月报":
                    ...
                    myDataTable = ClinkerMonthlyOutput.TableQuery(organizationId, time);
                    break;
                case "年报":
                    time = datetime.Substring(0, 4);
                    myDataTable = ClinkerYearlyOutput.TableQuery(organizationId, time);
                    break;
            }
            //myDataTable = ClinkerMonthlyOutput.TableQuery(organizationId, datetime);
