...
{
    public partial class table_CementMillMonthlyOutput : WebStyleBaseForEnergy.webStyleBase
    {
        private const string REPORT_TEMPLATE_PATH = "\\VBReportHeaderTemplate\\table_CementMillYearlyOutput.xml";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.OrganisationTree_ProductionLine.PageName = "table_CementMillMonthlyOutput.aspx";                                     //向web用户控件传递当前调用的页面名称
            ...
            if (m_FunctionName == "ExcelStream")
            {
                ...
                StatisticalReportHelper.ExportExcelFile("xls", "水泥（分品种）粉磨产量及消耗统计月报表.xls", m_HtmlData);
            }
        }
        ...
        public static string GetReportData(string organizationId, string datetime,string reportType)
        {
            ...
            switch (reportType)
            {
                case "月报":
                    ...
                    myDataTable = CementMillMonthlyOutput.TableQuery(organizationId, time);
                    break;
                case "年报":
                    time = datetime.Substring(0, 4);
                    myDataTable = CementMillYearlyOutput.TableQuery(organizationId, time);
                    break;
            }
            //myDataTable = CementMillMonthlyOutput.TableQuery(organizationId, datetime);
